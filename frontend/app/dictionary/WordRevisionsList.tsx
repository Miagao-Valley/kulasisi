    ...
    <Accordion type="single" collapsible>
      {revisions && revisions.length > 0 ? (
        revisions.map((revision, index) => {
          const prevContent = index < revisions.length - 1 ? revisions[index + 1].word : '';
            ...
            <AccordionItem value={`revision-${index}`} key={revision.history_id}>
              <AccordionTrigger>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground">modified word {naturalTime(revision.history_date)}</span>
                </div>
              </AccordionTrigger>
