...
export default async function GoogleTranslateCard({ text, source, target, className = '' }: Props) {
    ...
    <Card className={cn(className, `shadow-none'}`)}>
      <CardContent>
        <p className="text-base mb-2 whitespace-pre-line">
          <a href={data.google_translate_url || 'https://translate.google.com/'} target="_blank" className="hover:text-primary">
        ...
        <div className="flex gap-2 items-center">
          ...
          <span className="flex gap-1 text-xs text-muted-foreground">
            Translation from
            <a href="https://translate.google.com/" target="_blank" className="font-bold hover:underline underline-offset-4">
              Google Translate
            </a>
          </span>
        </div>
